...
import { getTimetableByTeacher } from "@/lib/api";
import { Video, Clock, MapPin, BookOpen, Calendar, Users } from "lucide-react";
...
export default function TeacherSchedule() {
  ...
  const { data: timetable = [], isLoading } = useQuery({
    ...
    queryFn: () => getTimetableByTeacher(user!.id),
    enabled: !!user?.id,
  });
  ...
  const todaySlots = (byDay[todayNum] || []);

  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-heading text-2xl font-bold">جدولي الدراسي</h1>
        <p className="text-muted-foreground text-sm mt-1">حصصك الأسبوعية في جميع الفصول</p>
      </div>

      {isLoading && <div className="text-center py-8 text-muted-foreground">جارٍ التحميل...</div>}

      {!isLoading && (timetable as any[]).length === 0 && (
        <div className="text-center py-12 text-muted-foreground bg-card rounded-lg border">
          <Calendar className="w-12 h-12 mx-auto mb-3 opacity-20" />
          <p className="font-heading">لم يتم إعداد جدولك الدراسي بعد</p>
          <p className="text-xs mt-1">تواصل مع الإدارة لإضافة جدولك</p>
        </div>
      )}
        ...
        <>
              ...
              <div className="flex gap-2 flex-wrap">
                {todaySlots.map((s: any) => (
                  <div key={s.id} className="bg-card border rounded-lg px-3 py-2 text-xs">
                    ...
                    <p className="text-muted-foreground">{s.classes?.name} — الصف {s.classes?.grade} / {s.classes?.section}</p>
                    <p className="text-muted-foreground">{s.start_time} - {s.end_time}</p>
                    {s.zoom_link && (
                      <a href={s.zoom_link} target="_blank" rel="noopener noreferrer"
                        className="inline-flex items-center gap-1 mt-1 text-info hover:underline">
          ...
          <div className="flex rounded-xl border overflow-hidden bg-card">
            {DAY_INDICES.map(day => (
              <button key={day} onClick={() => setSelectedDay(day)}
                ...
                <span>{DAYS[day]}</span>
                {day === todayNum && (
                  <span className={`absolute top-1.5 right-[calc(50%-10px)] w-1.5 h-1.5 rounded-full ${selectedDay === day ? "bg-white" : "bg-primary"}`} />
                )}
                <span className="text-xs block opacity-60 mt-0.5">{byDay[day]?.length || 0} حصص</span>
              </button>
            ))}
          </div>
          ...
          <div className="space-y-3">
            {daySlots.length > 0 ? daySlots.map((slot: any) => {
              ...
              return (
                <div key={slot.id} className={`bg-card rounded-xl border p-4 flex items-center gap-4 ${hasZoom ? "border-info/30" : ""}`}>
                  <div className={`w-12 h-12 rounded-xl flex flex-col items-center justify-center text-xs font-heading font-bold shrink-0 ${hasZoom ? "bg-info/10 text-info" : "bg-primary/10 text-primary"}`}>
                    <span className="text-base leading-none">{slot.period_number}</span>
                    <span className="font-normal opacity-70 mt-0.5">حصة</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-heading font-bold text-sm">{slot.subjects?.name}</h3>
                    <div className="flex flex-wrap items-center gap-3 mt-1">
                      <span className="flex items-center gap-1 text-xs text-muted-foreground">
                        <Users className="w-3 h-3" />{slot.classes?.name || `الصف ${slot.classes?.grade} / ${slot.classes?.section}`}
                      </span>
                      <span className="flex items-center gap-1 text-xs text-muted-foreground">
