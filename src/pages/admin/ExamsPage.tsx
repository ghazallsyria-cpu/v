...
import { getExams, addExam, updateExam, deleteExam, getClasses, getSubjects, getExamQuestions, addExamQuestion, updateExamQuestion, deleteExamQuestion, getExamResults, getStudentsByClass, getTeacherClasses, getTeacherSubjects, createNotification } from "@/lib/api";
...
export default function ExamsPage() {
  ...
  const togglePub = useMutation({
    ...
    onSuccess:(_,e)=>{ qc.invalidateQueries({queryKey:["exams"]}); if(sel?.id===e.id) setSel({...sel,is_published:!sel.is_published}); toast.success("تم تحديث حالة النشر"); if(!e.is_published){ getStudentsByClass(e.class_id).then(students=>{ (students as any[]).forEach(s=>createNotification({user_id:s.id,title:"اختبار جديد",body:`تم نشر اختبار: ${e.title}`,type:"exam"}).catch(()=>{})); }).catch(()=>{}); } }
  });
